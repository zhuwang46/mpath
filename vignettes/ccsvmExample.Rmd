...
```
The CC-family contains functions of composite of concave and convex functions. The CC-estimators are derived from minimizing loss functions in the CC-family via composite optimization by conjugation operator (COCO). The CC-estimators include robust support vector machines. See @wang2020unified.
...
```{r, eval=TRUE}
ccsvm.model <- ccsvm(y ~ ., data = dat, cost = 100, type="C-classification", cfun="acave", s=1)
summary(ccsvm.model)
...
ccsvm.opt <- cv.ccsvm(y ~ ., data=dat2, type="C-classification", s=1, cfun="acave", n.cores=2, balance=FALSE)
ccsvm.opt$cost
ccsvm.opt$gamma
ccsvm.opt$s
```
...
```{r, eval=TRUE}
ccsvm.model1 <- ccsvm(y ~ ., data = dat2, cost = ccsvm.opt$cost, gamma=ccsvm.opt$gamma, s=ccsvm.opt$s, cfun="acave", type="C-classification")
...
```{r, eval=TRUE}
svm.model1 <- svm(y~., data=dat2, cost=ccsvm.opt$cost, gamma=ccsvm.opt$gamma,  type="C-classification")
summary(svm.model1)
...
```{r}
ccsvm.model <- ccsvm(x=traindat[,-p], y=traindat[,p], cost = 2^3, gamma=2^(-4), epsilon=2^(-4), s=5, cfun="tcave")
summary(ccsvm.model)
...
plot(testdat[,p], ccsvm.predict, col="red", pch=1, ylab="Predicted values",xlab="Median home values ($1000)")
